...
class Builder(object):
    ...
    def get_top_10_indices(self, URM_row, nontarget_indices, known_indices, owner_indices):
        URM_row[nontarget_indices] = 0
        URM_row[known_indices] = 0
        URM_row[owner_indices] = 0
        return np.flip(np.argsort(URM_row)[-10:], 0)

    def get_top_10_tracks_from_indices(self, indices):
        tracks = self.get_tracks()
        top_10_tracks = [tracks[i] for i in indices]
